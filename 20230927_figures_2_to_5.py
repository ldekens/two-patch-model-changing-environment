...
"""
import numpy as np
import tools_figures_2_to_5 as tools
...
for g in G:
    ...
    ##### This section is for the plots illustrating the methods (fig2).
    if (g==0.7):
        C_fig2 = [0] #### environmental change speed
        tools.phase_lines_dominant_trait(C_fig2, parameters, parameters_trait_space, subworkdir)
    ...
    ###  To generate figure 4: first run numerical resolutions, next plot
    C =np.linspace(0., 2, num =10) #### Set environemental change speed
    if (g==1.4):
        C = np.append(C, 2 - 2/9*3/2)  ### add a data point to capture interestinf phenomenon
    if (g==1.8):
