...
####### Some functions are exactly the same as in tools_figures_2_to_5.py, where they are generally better commented
...
##### Function that displays the figure 7
def phase_lines_all_speed(parameters, parameters_trait_space, subworkdir):
    ...
    c = F_dp
    if (g!=1.3):
        plt.text(z_dp+0.16, F_dp-0.07, r'$\boldsymbol{c_{\text{death plain}}}$', color = colors[count] ,fontsize = 29)
    ...
    if (g>1):
        ...
        if (g!=1.3):
            plt.text(z_dv+0.25, c_dv-0.07, r'$\boldsymbol{c_{\text{death valley}}}$', color = colors[count] ,fontsize = 29)
        if (g==1.3):
            plt.text(z_dv+0.25, c_dv-0.18, r'$\boldsymbol{c_{\text{death valley}}}$', color = colors[count] ,fontsize = 29)
    ...
    plt.yticks(fontsize = 30)
    z_star, rho_star, F_z_star = curve_per_parameter(parameters, parameters_trait_space)
    #plt.vlines(0, -1.8, 1.8, color = 'black')
    plt.vlines(z_star[np.argmax([F_z_star<0])], -0.05, 0.05, color ='black')
    plt.vlines(0, -0.05, 0.05, linewidth =2,  color ='black')
    plt.text(-0.07, 0.1, '0', fontsize = 30)
    viability = viability_fast_equlibria_theoric(parameters, z_star)
    epsilon, r, kappa, g, m, theta = parameters
    colors = [inferno(50), 'darkred']
    count = 0
    plt.scatter(z_star[- np.argmax([F_z_star[::-1]>0])], 0, s=200., color = colors[count])
    plt.plot(Z[(Z-1)**2<1.4], -g*(Z[(Z-1)**2<1.4]-1)**2, linewidth = 8, alpha = 0.2, color = viridis(200))
    plt.plot(Z[(Z+1)**2<1.4], -g*(Z[(Z+1)**2<1.4]+1)**2, linewidth = 8, alpha = 0.2, color = viridis(70))
    ...
    count = count +1
    c = F_dp
    #plt.scatter(z_dp-0.02, 0, s=200., color = colors[count])
    #plt.text(z_dp-0.1, 0- 0.22, r'$ Z^*_{\text{death plain}}$', fontsize = 29, color = colors[count])

    plt.text(z_dp+0.16, F_dp, r'$\boldsymbol{c_{\text{death plain}}}$', color = colors[count] ,fontsize = 29)
    plt.hlines(F_dp, z_dp - 0.15, z_dp + 0.15, color = colors[count], linewidth =5)
    #plt.vlines(z_dp, 0, F_dp, color = colors[count], linewidth =2)
    plt.plot(Z, np.zeros(np.size(Z)), color='black', linewidth=2.5)
    ...
    if (g==1.4):
        ...
        plt.text(-z_dv+0.11, 4.4, s= r'VALLEY', fontsize = 30)
        plt.fill_betweenx(y=[-2.8, 5.4], x1=-z_dv, x2=z_dv, color = 'black', alpha = 0.3)
    #plt.legend(loc=0, markerscale= 8, fontsize=15)
    plt.savefig(subworkdir + "/phase_lines_death_plain.png", format='png', bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    if (g>1):
        ...
        plt.text(-0.07, 0.1, '0', fontsize = 30)
        viability = viability_fast_equlibria_theoric(parameters, z_star)
        colors = [inferno(50), 'chocolate', 'RoyalBlue']
        count = 0
        plt.text(z_star[-np.argmax([F_z_star[::-1]>0])], 0.15, r'$ Z^*_{\text{spec}}$', fontsize = 30, color = inferno(0))
        plt.text(z_star[np.argmax([F_z_star<0])]-0.1, 0.15, r'$ -Z^*_{\text{spec}}$', fontsize = 30, color = inferno(0))
        plt.plot(z_star[viability&(z_star>=0)], F_z_star[viability&(z_star>=0)], linewidth = 3, color = colors[count])
        ...
        c = F_dp
        #plt.scatter(z_dp-0.02, 0, s=200., color = colors[count])
        #plt.text(z_dp-0.1, 0- 0.2, r'$ Z^*_{\text{death valley}}$', fontsize = 29, color = colors[count])
        Delta = 4/g**2*(m**2-4*g*(m-1))
        z_dv = np.sqrt(1/2*(2*(1+g-m)/g - np.sqrt(Delta)))
        c_dv = 2*g*z_dv*(-1 + 2/(z_dv**2+1+(m-1)/g) )
        z_dp = -np.sqrt(1/2*(2*(1+g-m)/g + np.sqrt(Delta)))
        ...
        F_max = np.max(F_z_star[z_star>=0])
        plt.hlines(F_max, z_max - 0.2, z_max + 0.2, color = colors[count], linewidth =5)
        plt.text(z_max + 0.2, F_max+0.1, r'$\boldsymbol{c_{\text{switch}}}$',color = colors[count] ,fontsize = 30)
        plt.plot(Z[(Z-1)**2<1.4], -g*(Z[(Z-1)**2<1.4]-1)**2, linewidth = 8, alpha = 0.2, color = viridis(200))
        plt.plot(Z[(Z+1)**2<1.4], -g*(Z[(Z+1)**2<1.4]+1)**2, linewidth = 8, alpha = 0.2, color = viridis(70))
        #plt.legend(loc=0, markerscale= 8, fontsize=15)
        plt.savefig(subworkdir + "/phase_lines_death_plain.png", format='png', bbox_inches='tight')
    ...
    plt.hlines(F_max, zmin, zmax, color = colors[count], linewidth =7)
    ...
    plt.hlines(F_dp, zmin, zmax, color = colors[count], linewidth =7) 
    ...
    if (g>1):
        ...
        plt.hlines(c_dv, zmin, zmax, color = colors[count], linewidth =7) 
    plt.plot(Z[(Z-1)**2<1.4], -g*(Z[(Z-1)**2<1.4]-1)**2, linewidth = 8, alpha = 0.2, color = viridis(200))
    plt.plot(Z[(Z+1)**2<1.4], -g*(Z[(Z+1)**2<1.4]+1)**2, linewidth = 8, alpha = 0.2, color = viridis(70))
    plt.plot(Z, np.zeros(np.size(Z)), color='black', linewidth=2.5)
    Delta = 4/g**2*(m**2-4*g*(m-1))
    ...
    if (g==1.3):
        ...
        plt.fill_betweenx(y=[-2.8, 5.4], x1=-z_dv, x2=z_dv, color = 'black', alpha = 0.3)    
    #plt.legend(loc=0, markerscale= 8, fontsize=15)
    plt.savefig(subworkdir + "/phase_lines_summary_critical_speeds.png", format='png', bbox_inches='tight')
    plt.show()
    plt.close()
#### Function to build fig 8
...
def plot_critical_speed_th(G, parameters, parameters_trait_space, subworkdir):
    ...
    plt.plot(G[(np.sqrt(z1)<Zstar)&(G>=1)], c1[(np.sqrt(z1)<Zstar)&(G>=1)], color ='chocolate', linewidth = 5, label = r'Death Valley')
    plt.plot(G[(np.sqrt(z1)>Zstar)&(G>=1)], c1[(np.sqrt(z1)>Zstar)&(G>=1)], color ='chocolate', linewidth = 5 )
